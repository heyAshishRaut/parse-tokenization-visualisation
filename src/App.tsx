            ...
            <div className="h-[90%] w-full flex flex-col md:flex-row gap-4 px-[10px] md:px-[50px] lg:px-[120px] py-4 pb-[100px] md:pb-0">
                ...
                <div className={`h-full w-full md:w-1/2`}>
                    ...
                    <div className="w-full flex flex-col gap-4">
                        <div style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} className={`${darkmode ? "text-white" : "text-black"} px-3 py-2 font-mono h-[280px] w-full rounded-lg border-[2px] ${darkmode ? "border-neutral-700/70" : "border-gray-300"} `}>
                            {decoded.map((e, i) => (
                                <span key={i} className="px-2 font-mono">{e}</span>
                            ))}
                        </div>
                        <div style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} className={`${darkmode ? "text-white" : "text-black"} px-2 py-3 h-[280px] w-full rounded-lg border-[2px] ${darkmode ? "border-neutral-700/70" : "border-gray-300"} `}>
                            {nums.map((e, i) => (
                                <span key={i} className="px-2 font-mono">{e}</span>
                            ))}
                        </div>
